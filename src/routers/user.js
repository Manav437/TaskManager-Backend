...
router.post("/users", async (req, res) => {
    ...
    try {
        ...
        // Send a clean, simple error to the client
        res.status(400).send({
...
router.post("/users/login", async (req, res) => {
    try {
        const user = await User.findByCredentials(
            req.body.email,
            req.body.password,
        ); //this is custom method made by us, to check credentials
        const token = await user.generateAuthToken();
...
router.get("/users/me", auth, async (req, res) => {
    //fetch all users ---> (changed to fetch profile on user that is logged in)
...
router.patch("/users/me", auth, async (req, res) => {
    //if you try to update a property that doesnt exist, 200 status(i.e no error) but wont reflect in databse
    ...
    const isValidOperation = updates.every((update) => {
        //isValOpe is a boolean var, if one single value is false, .every() will return false overall
    ...
    try {
        ...
        updates.forEach((update) => {
            // (user koi si bhi property update kar skta hai, isliye bracket notation)
