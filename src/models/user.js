...
const userSchema = new mongoose.Schema({ // we used 'User' as the name but once we check in 3T it is 'users',bcoz
    ...
    age: {
        ...
        validate(value) {               //this is a custom validator, value is the query given by the user
            if (value < 0) {
    ...
    password: {
        ...
        validate(value) {
            if (value.toLowerCase().includes('password')) {         //toLowerCase is used bcoz the user may type "PASSWORD" yk :)
                throw new Error('the password cant contain the string - password')
            }
        }
    },
    tokens: [{
...
userSchema.methods.toJSON = function () {                     //2nd method
    const user = this
...
userSchema.methods.generateAuthToken = async function () {          //fxn has to be normal fxn
    const user = this
...
userSchema.pre('save', async function (next) {          //'save' is the name of the event, then the function(dont use arrow fxn[bcoz of 'this' binding])
    const user = this
